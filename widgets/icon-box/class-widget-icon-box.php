...
namespace Xoom_Addons\Widgets\Icon_Box;
 ...
 */
class Widget_Icon_Box extends Base_Widget {
	...
	public function get_title() {
		return __( 'Icon Box', 'xoom-addons-for-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_content',
			array(
				'label' => __( 'Icon Box', 'xoom-addons-for-elementor' ),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box' => 'text-align: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .xoom-icon-box',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .xoom-icon-box' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .xoom-icon-box' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#5b4bff',
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box__icon'     => 'color: {{VALUE}};',
					'{{WRAPPER}} .xoom-icon-box__icon svg' => 'fill: {{VALUE}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box__icon'     => 'font-size: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .xoom-icon-box__icon svg' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box--left'                       => 'gap: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .xoom-icon-box--right'                      => 'gap: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .xoom-icon-box--top .xoom-icon-box__content' => 'margin-top: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box__title' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .xoom-icon-box__title',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'separator' => 'before',
				'selectors' => array(
					'{{WRAPPER}} .xoom-icon-box__desc' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .xoom-icon-box__desc',
			)
		);
	...
	protected function render() {
		...
		if ( $has_link ) {
			...
			$this->add_render_attribute( 'link', 'class', 'xoom-icon-box__link' );
		}
		?>
		<div class="xoom-icon-box xoom-icon-box--<?php echo esc_attr( $position ); ?>">
			<?php if ( $has_icon ) : ?>
				<span class="xoom-icon-box__icon" aria-hidden="true">
					<?php Icons_Manager::render_icon( $settings['icon'], array( 'aria-hidden' => 'true' ) ); ?>
				</span>
			<?php endif; ?>

			<div class="xoom-icon-box__content">
				<?php if ( '' !== $title ) : ?>
					<h3 class="xoom-icon-box__title">
						<?php if ( $has_link ) : ?>
				...
				<?php if ( '' !== $desc ) : ?>
					<div class="xoom-icon-box__desc"><?php echo wp_kses_post( $desc ); ?></div>
				<?php endif; ?>
			</div>
		</div>
		<?php
